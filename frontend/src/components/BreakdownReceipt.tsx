...
export function BreakdownReceipt({ metrics, receipt }: BreakdownReceiptProps) {
  const getGradeColor = (grade: string) => {
    if (grade === 'A+' || grade === 'A' || grade === 'A-') return 'text-green-400';
    if (grade === 'B+' || grade === 'B' || grade === 'B-') return 'text-emerald-400';
    if (grade === 'C+' || grade === 'C' || grade === 'C-') return 'text-yellow-400';
    if (grade === 'D+' || grade === 'D' || grade === 'D-') return 'text-orange-400';
    return 'text-red-400';
  };
  ...
  return (
    <div className="text-gray-100 font-mono text-sm space-y-3">
      <div className="text-center border-b border-white/10 pb-2">
        ...
        <p className="text-gray-400 text-xs mt-0.5">True Cost Analysis</p>
      </div>
      ...
      <div className="border-t border-white/10 pt-3 bg-white/5 -mx-5 px-5 pb-4 rounded-b-2xl">
        <div className="flex justify-between items-center">
          ...
          <span className={`text-4xl ${gradeColor}`}>{metrics.grade}</span>
        </div>
